...
with col_2:
    st.title('Dashboard di Produzione')
...
with tab2:
    ...
    # Selezione del periodo di analisi con slider
    ...
    lean_filtered = lean_filtered[(lean_filtered['DATA_CHIUSURA'] >= date_range[0]) & (lean_filtered['DATA_CHIUSURA'] <= date_range[1])]
    ...
    def grafico_scostamenti_z_score(df):
        ...
        fig_scostamenti.update_layout(showlegend=False)
        return fig_scostamenti
...
with tab1:
    ...
    st.markdown("---")
    st.markdown("### Andamento FF stampati 2024 - YTD")
    ...
    ultimo_periodo = df_reparto['Periodo'].max()
